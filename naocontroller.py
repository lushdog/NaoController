...
class NaoController(cmd.Cmd):
    ...
    def do_connect(self, arg):
        '''Connect to nao robot - 'connect <ip/host of robot> <port>' or 'connect' alone (uses defaults.py)'''
    ...
    def do_move(self, arg):
        '''Rotates the robot and then moves it forward or backward - 'move <rotation in clock hours> <distance in meters to travel forweard or backward>' '''
        if (not self.is_connected):
            self.print_not_connected_error()
            return
    ...
    def format_say_inputs(self, animation, speech):
        animated_speech = '^startTag({0}) "\\rspd={2}\\{1}" ^waitTag({0})'.format(animation.lower(), speech, defaults.SPEECH_SPEED)
    ...
    def process_move_arg(self, arg):
        ...
        '''arg[1] - distance in meters to move forward'''
        '''returns arg[0] - relative rotation in radians'''
        '''returns arg[1] - distance in meters to move, clamped between -10 and 10'''
    ...
    def convert_hour_to_radians(self, hour):
    ...
    def clamp(self, minvalue, value, maxvalue):
